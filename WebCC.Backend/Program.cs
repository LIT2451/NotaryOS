...
var connectionString = Environment.GetEnvironmentVariable("WEBCC_DB_CONNECTION")
    ?? builder.Configuration.GetConnectionString("DefaultConnection");

if (string.IsNullOrWhiteSpace(connectionString))
{
    throw new InvalidOperationException("Missing database connection string. Set WEBCC_DB_CONNECTION or ConnectionStrings:DefaultConnection.");
}
...
{
    options.SwaggerDoc("v1", new OpenApiInfo { Title = "WEBCC API", Version = "v1" });
...
var jwtToken = Environment.GetEnvironmentVariable("WEBCC_JWT_TOKEN")
    ?? builder.Configuration.GetSection("AppSettings:Token").Value;

if (string.IsNullOrWhiteSpace(jwtToken))
{
    throw new InvalidOperationException("Missing JWT token secret. Set WEBCC_JWT_TOKEN or AppSettings:Token.");
}
...
using (var scope = app.Services.CreateScope())
{
    ...
    await WebCC.Backend.Data.DbSeeder.SeedRbacAsync(scope.ServiceProvider);
}
